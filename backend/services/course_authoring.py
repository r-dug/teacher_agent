    ...
    *,
    ...
    await conn.execute(
        """INSERT INTO course_advisor_sessions
           (course_id, user_id, transcript_json, objectives_prompt, status, created_at, updated_at)
...
async def finalize_advisor_session(
    ...
    *,
    ...
    def _extract_preview() -> str:
        from shared.pdf_tools import extract_text_plain
    ...
    *,
    ...
    await conn.execute(
        """INSERT INTO course_decomposition_jobs
           (id, course_id, user_id, status, objectives_prompt, total_items, completed_items, failed_items,
...
def _decompose_chapter_sync(
    ...
    if provider == "openai":
        ...
        agent = TeachingAgent(llm_model=decompose_model)
        client = anthropic.Anthropic(max_retries=6)
        supports_thinking = "haiku" not in decompose_model
        _, sections = agent._decompose_segment_bytes(  # noqa: SLF001 - intentional reuse of core decomposition logic
            client,
            pdf_bytes,
            seg_start,
            seg_end,
...
def _decompose_chapter_openai_sync(
    *,
    ...
    import fitz
    from shared.teaching_agent import DECOMPOSE_PROMPT, DECOMPOSE_SYSTEM
...
async def _upsert_chapter_lesson(
    *,
    ...
    if lesson is None or str(lesson.get("user_id")) != user_id:
